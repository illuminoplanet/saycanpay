...
from saycanpay.data_utils import process_action, ExceededMaxSteps
...
class CanModel(pl.LightningModule):
            ...
            [
                {
                    ...
                    "lr": self.lr,
                },  # e.g. learning rate for the classifier
                {
                    "params": base_model_params,
                    "lr": 1e-5,
                },  # e.g. learning rate for the base model
            ],
    ...
    def training_step(self, train_batch, batch_idx):
        context_pos_action = [
            c + " [NXT] " + a
            for c, a in zip(train_batch["context"], train_batch["pos_action"])
        ]
        context_neg_action = [
            c + " [NXT] " + a
            for c, a in zip(train_batch["context"], train_batch["neg_action"])
        ]
        context_neg_sample_action = [
            c + " [NXT] " + a
            for c, a in zip(train_batch["context"], train_batch["neg_sample_action"])
        ]
        ...
        y = torch.cat(
            (
                torch.ones_like(pos_probs),
                torch.zeros_like(neg_probs),
                torch.zeros_like(neg_sample_probs),
            ),
            dim=0,
        )
    ...
    def on_train_epoch_end(self):
        train_acc, train_f1 = list(self.train_metrics.compute().values())
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        context_pos_action = [
            c + " [NXT] " + a
            for c, a in zip(val_batch["context"], val_batch["pos_action"])
        ]
        context_neg_action = [
            c + " [NXT] " + a
            for c, a in zip(val_batch["context"], val_batch["neg_action"])
        ]
        context_neg_sample_action = [
            c + " [NXT] " + a
            for c, a in zip(val_batch["context"], val_batch["neg_sample_action"])
        ]
        pos_probs = self.classifier(self.encode(context_pos_action))
        neg_probs = self.classifier(self.encode(context_neg_action))
        neg_sample_probs = self.classifier(self.encode(context_neg_sample_action))
    ...
    def on_validation_epoch_end(self):
        val_acc, val_f1 = list(self.val_metrics.compute().values())
...
class PayModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optimizer = optim.AdamW(
            [
                {
                    "params": value_model_params,
                    "lr": self.lr,
                },  # e.g. learning rate for the value_model
                {
                    "params": base_model_params,
                    "lr": 1e-5,
                },  # e.g. learning rate for the base model
    ...
    def training_step(self, train_batch, batch_idx):
        context_action = [
            c + " [NXT] " + a
            for c, a in zip(train_batch["context"], train_batch["action"])
        ]
...
class CustomScorer:
    def __init__(self, decoding_score, model_name):
        ...
        can_ckpt = os.path.join(
            os.environ["CKPT_ROOT"], "can", "epoch=23-step=144.ckpt"
        )
        ...
        pay_ckpt = os.path.join(
            os.environ["CKPT_ROOT"], "pay", "epoch=15-step=96-v2.ckpt"
        )
        self.device = (
    ...
    def filter_beams(
        ...
        # print('\n')
        for ind, (gen_sequences, lm_scores) in enumerate(
            zip(generated_sequences, generated_beam_scores)
        ):
            for gen_seq, lm_score in zip(gen_sequences, lm_scores):
                ...
                if self.decoding_score == "say_can":
                    with torch.no_grad():
                        pre_score = self.precondition_score(
                            context=prev_contexts_can[ind], action=f"{gen_seq}."
                        )
                    # all log scores
                    # print(gen_seq)
                    ...
                    with torch.no_grad():
                        pre_score, post_score = self.precondition_score(
                            context=prev_contexts_can[ind], action=f"{gen_seq}."
                        ), self.postcondition_score(
                            context=prev_contexts_pro[ind], action=f"{gen_seq}."
                        )
                    # all log scores
                    # print(gen_seq)
                    ...
                    raise Exception(
                        "Not a valid decoding score; check decoding_score argument in config file"
                    )
                ...
                new_context_can = (
                    f"{prev_contexts_can[ind]} [Step {curr_action_step}] {gen_seq}."
                )
                new_context_pro = (
                    f"{prev_contexts_pro[ind]} [Step {curr_action_step}] {gen_seq}."
                )
                updated_action_beam_scores.append(new_beam_score)
                updated_action_beam_prompts.append(new_beam_prompt)
                (
                    done.append(True)
                    if task_completed_desc in gen_seq
        ...
        )  # selecting num_final_beams
        return (
            list(action_beam_scores),
            list(action_beam_prompts),
            list(done_updated),
            list(can_contexts_updated),
            list(pro_contexts_updated),
            list(updated_generated_sequences),
        )


class LmPlannerBase(ABC):
    def __init__(
        self, env, model_name, decoding_type, decoding_score, oracle_flag, max_steps
    ):
        self.env = env
        self.lm_model = None
        self.tokenizer = None
        ...
        self.num_beams = (
            6  # 8, 6  # divided into num_beam_groups (2 sequences in each group)
        )
        ...
        self.diversity_penalty = (
            2.0  # higher the diversity penalization, lower the diversity
        )
        ...
        self.oracle_flag = (
            oracle_flag  # oracle agent is used to collect expert trajectories
        )
        self.model_name = model_name
    ...
    def __init_lm(self):
        ...
        if self.model_name == "vicuna":
            self.tokenizer = LlamaTokenizer.from_pretrained(
                "eachadea/vicuna-13b-1.1", padding_side="left"
            )
            self.tokenizer.add_special_tokens({"pad_token": "[PAD]"})
            self.lm_model = LlamaForCausalLM.from_pretrained(
                "eachadea/vicuna-13b-1.1", device_map="auto", load_in_8bit=True
            )
            ...
            self.lm_model = T5ForConditionalGeneration.from_pretrained(
        ...
        if self.decoding_type != "greedy_token":
            self.custom_scorer = CustomScorer(
                decoding_score=self.decoding_score, model_name=self.model_name
            )
        ...
        if self.decoding_type == "greedy_token":
            ...
            _, _, _, _, _, generated_sequences_updated = (
                self.custom_scorer.filter_beams(
                    ...
                    [generated_beam_scores],
                    action_beam_prompts=[
                        prompt for _ in range(len(generated_sequences))
                    ],
                    action_beam_scores=[0 for _ in range(len(generated_sequences))],
                    prev_contexts_can=[prev_context_can],
                    prev_contexts_pro=[prev_context_pro],
                    goal=goal,
            ...
            action = generated_sequences_updated[
                0
            ]  # action with the highest score (already sorted in filter beams)
        else:
            raise Exception(
                "Not a valid decoding type; check decoding_type argument in config file"
            )
        return action
    ...
    def lm_planner(self, prompt, admissible_actions, context_can, context_pro, goal):
        ...
        assert (
            self.task_completed_desc is not None
        ), "please initialize the task_completed_desc argument"
        ...
        if not beam_actions_flag:
            ...
            def postprocess_plans_from_beams(beam_prompt):
                ...
                if "Initial State" in beam_prompt:
                    ...
                    beam_prompt = beam_prompt.split("[Goal] ")[
                        -1
                    ].strip()  # virtualhome
            ...
            generated_sequences, generated_beam_scores = self.select_unique_steps(
            ...
            while True:  # at least one action sequence has not terminated
                ...
                if action_steps > self.max_steps:
                    ...
                    if len(all_beams) > 0:
                        ...
                        raw_plan = sorted(all_beams, key=lambda x: x[1], reverse=True)[
                            0
                        ][0]
                        plan = ""
                        step_ind = 1
                ...
                new_generated_sequences, new_generated_beam_scores = self.beam_generate(
                    action_beam_prompts
                )
                # postprocess
                new_generated_sequences = [
                    process_action(seq, admissible_actions)
                ...
                for gen_id, gen_seqs in enumerate(new_generated_sequences):
                    ...
                    # filter out repeated steps
                    unique_gen_seqs, unique_gen_beam_scores = self.select_unique_steps(
