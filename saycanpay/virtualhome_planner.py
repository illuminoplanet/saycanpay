...
from saycanpay.lm_planner_base import LmPlannerBase
from .data_utils import (
    ExceededMaxSteps,
    process_saved_plans_virtualhome,
    latest_file_from_dir,
)
...
class LmAgent(LmPlannerBase):
    def __init__(
        ...
        super().__init__(
            env, model_name, decoding_type, decoding_score, oracle_flag, max_steps
        )
        self.task_completed_desc = "done task"

    def __init_logs(self, save_dir):
        self.plan_log_file = open(
            os.path.join(save_dir, time.strftime("%Y-%m-%d %H:%M")), "w"
        )
        ...
        self.oracle_plans = process_saved_plans_virtualhome(
            filepath=oracle_traj_file, task_completed_desc=self.task_completed_desc
        )
        admissible_action_path = os.path.join(
            os.environ["VIRTUALHOME"], "admissible_actions"
        )
    ...
    def initialize_episode(self, ep):
        ...
        info = {
            "lang_goal": saved_plan.goal,
            "initial_state": initial_state,
            "graph_id": saved_plan.graph_id,
    ...
    def env_step(self, actions, **kwargs):
        ...
        if not self.record:
            done, message_exec = self.env.render_script(
                script=script,
    ...
    def test_plans(self, genplan_file_path):
        ...
        gen_plans = process_saved_plans_virtualhome(
            filepath=genplan_file_path, task_completed_desc=self.task_completed_desc
        )
